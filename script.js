document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- RESPONSIV VIDEO-VÄXLARE ---
    const heroVideoTag = document.getElementById('heroVideo');
    if (heroVideoTag) {
        const desktopSrc = heroVideoTag.querySelector('source').getAttribute('src');
        const mobileSrc = heroVideoTag.dataset.mobileSrc;

        if (window.innerWidth < 768 && mobileSrc) {
            if (heroVideoTag.querySelector('source').getAttribute('src') !== mobileSrc) {
                heroVideoTag.querySelector('source').setAttribute('src', mobileSrc);
                heroVideoTag.load();
            }
        }
    }
    ...
    if (music) {
        music.volume = 0.1;
        music.play().catch(() => {
            console.log("Autoplay av musik blockerades av webbläsaren.");
        });
